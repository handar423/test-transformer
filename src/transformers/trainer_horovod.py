...
class HorovodTrainer:
    ...
    def __init__(
        ...
        **kwargs,
        ...
        args.warmup_steps = max(40, args.warmup_steps)
        args.local_rank = hvd.local_rank()
        args.world_rank = hvd.rank()

        # 此处我们跳过setup_device方法，因为它会自动启用torchDDP
        # 同时要求改动后的脚本不能使用local_rank参数
    ...
    def _get_train_sampler(self) -> Optional[torch.utils.data.sampler.Sampler]:
        if isinstance(self.train_dataset, torch.utils.data.IterableDataset):
            ...
            return (
                ...
                else DistributedSampler(self.train_dataset, num_replicas=hvd.size(), rank=hvd.rank())
            )
    ...
    def _get_eval_sampler(self, eval_dataset: Dataset) -> Optional[torch.utils.data.sampler.Sampler]:
        if isinstance(eval_dataset, torch.utils.data.IterableDataset):
            ...
            return SequentialDistributedSampler(eval_dataset, num_replicas=hvd.size(), rank=hvd.rank())
        else:
    ...
    def train(self, model_path: Optional[str] = None, trial: Union["optuna.Trial", Dict[str, Any]] = None):
        ...
        if is_torch_tpu_available():
            ...
            total_train_batch_size = (
                ...
                * (hvd.size() if self.args.local_rank != -1 else 1)
            )
        ...
        # 在下面这个循环中加入计时相关log，我们目前只记录forward开始时间以记录batch_time
        for epoch in range(epochs_trained, int(np.ceil(num_train_epochs))):
            if isinstance(train_dataloader, DataLoader) and isinstance(train_dataloader.sampler, DistributedSampler):
                train_dataloader.sampler.set_epoch(epoch)
            if is_torch_tpu_available():
                parallel_loader = pl.ParallelLoader(train_dataloader, [self.device]).per_device_loader(
                    self.device
                )
            ...
            for step, inputs in enumerate(epoch_iterator):
                ...
                tmp_loss = self.training_step(model, inputs)
                self.lobj = {"name": "loss", "val": tmp_loss.item(), "pid": hvd.rank()}
    ...
    def compute_loss(self, model, inputs):
        ...
        """
        # forward
        self.lobj = {"ph": "X", "name": "foward", "ts": time.time(), "pid": hvd.rank(), "dur": 0}
    ...
    def is_world_process_zero(self) -> bool:
        ...
        if is_torch_tpu_available():
            ...
            return self.args.local_rank == -1 or self.args.world_rank == 0
    ...
    def prediction_loop(
        ...
        if self.compute_metrics is not None and preds is not None and label_ids is not None:
            ...
            metrics = {}
        # TODO 先跳过predict的部分，之后再添加，因为此处比较的是training scaling factor
