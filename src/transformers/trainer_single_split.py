...
class SplitSingleTrainer:
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.model_pre = model_pre.to(torch.device("cpu")) if model_pre is not None else None
        self.model_suf = model_suf.to(args.device) if model_suf is not None else None
        if args.model_split=="head_mask":
            self.split_strategy = 0
        elif args.model_split=="embedding" :
            self.split_strategy = 1
        elif args.model_split=="single_hidden" :
            self.split_strategy = 2
        elif args.model_split=="encoder" :
            self.split_strategy = 3
        else:
            self.split_strategy = -1
    ...
    def _prepare_inputs_cpu(self, inputs: Dict[str, Union[torch.Tensor, Any]]) -> Dict[str, Union[torch.Tensor, Any]]:
        """
        Prepare :obj:`inputs` before feeding them to the model, converting them to tensors if they are not already and
        handling potential state.
        """
    ...
    def prediction_step_head_mask(
        self, model_pre, model_suf: nn.Module, inputs: Dict[str, Union[torch.Tensor, Any]], prediction_loss_only: bool
    ) -> Tuple[Optional[float], Optional[torch.Tensor], Optional[torch.Tensor]]:
        """
        Perform an evaluation step on :obj:`model` using obj:`inputs`.
