...
class BwdTestTrainer:
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.lobj = {}
        self.allEvent = {}
        self.Flag_event = torch.cuda.Event(enable_timing=True)
        self.bwd_fout = open("bwd_order.log", "w")
    ...
    def create_optimizer_and_scheduler(self, num_training_steps: int):
        ...
        if self.optimizer is None:
            ...
            # insert hook
            def get_hook_event(name):
                self.allEvent[name] = torch.cuda.Event(enable_timing=True)
                def _hook(*ignore):
                    self.allEvent[name].record()
                return _hook

            for n, p in self.model.named_parameters():
                if p.requires_grad:
                    p.register_hook(get_hook_event(n))
    ...
    def train(self, model_path: Optional[str] = None, trial: Union["optuna.Trial", Dict[str, Any]] = None):
        ...
        for epoch in range(epochs_trained, int(np.ceil(num_train_epochs))):
            ...
            for step, inputs in enumerate(epoch_iterator):
                ...
                if (self.global_step + 5) == self.args.max_steps:
                    Flag_event = torch.cuda.Event(enable_timing=True)
                    Flag_event.record()
                    torch.cuda.synchronize()
                    time_dict = {n : self.allEvent[n].elapsed_time(Flag_event) 
                                 for n in self.allEvent.keys() if self.allEvent[n].cuda_event != 0}
                    time_dict = sorted(time_dict.items(), key=lambda x: x[1], reverse=True)
                    pprint("new step", stream=self.bwd_fout)
                    pprint(time_dict, stream=self.bwd_fout)
